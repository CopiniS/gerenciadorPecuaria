<template>
  <div>
    ...
    <div class="modal fade" id="confirmacaoExclusaoModal" tabindex="-1" aria-labelledby="confirmacaoExclusaoModalLabel"
      aria-hidden="true" v-if="exibirModalPropriedades">
...
export default {
  ...
  methods: {
    ...
    async apagarPropriedade() {
      try {
        const response = await axios.delete(`http://127.0.0.1:8000/propriedades/ ${this.formData.id}/`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('access_token')}`
        }
      })
    ...
    async submitFormEdicao() {
      try {
        ...
        const response = await axios.patch(`http://127.0.0.1:8000/propriedades/${this.formData.id}/`, dadosPropriedade, {
          headers: {
          'Authorization': `Bearer ${localStorage.getItem('access_token')}`
          }}
        );
