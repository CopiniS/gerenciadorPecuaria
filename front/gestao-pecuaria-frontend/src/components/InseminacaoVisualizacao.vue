...
<div class="background">
  ...
  <div class="nav nav-tabs" id="nav-tab" role="tablist">
    <button class="nav-link" :class="{ active: activeTab === 'inseminacoes' }" id="nav-vet-tab" @click="selectTab('inseminacoes')" 
    type="button" role="tab" aria-controls="nav-vet" aria-selected="true">Lista de Inseminação</button>
...
<h2>Vizualização</h2>
  ...
  <div>
    <div class="table-container">
        <p><strong>Data da Inseminacao:</strong> {{ formatarData(this.dataSelecionada) }}</p>
            <table class="table table-striped">
                ...
                <tbody>
                  <tr v-for="inseminacao in this.inseminacoesSelecionadas" :key="inseminacao.id">
  ...
  mounted() {
    const dataSelecionada = this.$route.params.dataSelecionada;
        if (dataSelecionada) {
            ...
            this.fetchInseminacoes();
        }
  },
  methods: {

    async fetchInseminacoes() {
      try {
        const response = await api.get(`http://127.0.0.1:8000/inseminacoes/por-data/`, {
          params: {
                propriedadeSelecionada: localStorage.getItem('propriedadeSelecionada'),
                dataSelecionada: this.dataSelecionada
            },
        });
        this.inseminacoesSelecionadas = response.data;
        
        if(this.inseminacoesSelecionadas.length === 0){
    ...
    async apagarInseminacao() {
      try {
        ...
        if (response.status === 204) {
          alert('Inseminacao excluído com sucesso!');
          this.fetchInseminacoes();
          this.inseminacaoParaExcluir = null;
        } else {
          alert('Erro ao excluir a inseminacao. Tente novamente mais tarde.');
        }
      } catch (error) {
