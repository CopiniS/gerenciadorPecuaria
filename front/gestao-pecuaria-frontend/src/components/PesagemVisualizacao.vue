...
<div class="background">
  ...
  <div class="nav nav-tabs" id="nav-tab" role="tablist">
    ...
    type="button" role="tab" aria-controls="nav-vet" aria-selected="true">Lista de Pesagens</button>
    ...
    <div class="table-container">
        <p><strong>Data da Pesagem:</strong> {{ formatarData(this.dataSelecionada) }}</p>
            <table class="table table-striped">
              ...
              <tbody>
                <tr v-for="pesagem in this.pesagensSelecionadas" :key="pesagem.id">
...
export default {
  ...
  methods: {
    ...
    async fetchPesagens() {
      try {
        const response = await api.get(`http://127.0.0.1:8000/pesagens/por-data/`, {
          params: {
                propriedadeSelecionada: localStorage.getItem('propriedadeSelecionada'),
                dataSelecionada: this.dataSelecionada
            },
        });
        this.pesagensSelecionadas = response.data;
        
        if(this.pesagensSelecionadas.length === 0){
