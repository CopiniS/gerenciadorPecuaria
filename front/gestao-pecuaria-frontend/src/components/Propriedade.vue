          ...
          <div class="modal-body">
            <form id="formProdutor" @submit.prevent="submitEditForm">
              ...
              <div class="mb-3 input-group">
                ...
                <input v-model="formData.endereco" type="text" class="form-control" placeholder="Endereço"
                  id="enderecoEditar">
              </div>
              <div class="mb-3 input-group">
              ...
              <div class="mb-3 input-group">
                ...
                <input v-model="formData.latitude" type="text" class="form-control" placeholder="Latitude"
                  id="latitudeEditar">
              </div>
              <div class="mb-3 input-group">
                <span class="input-group-text"><i class="fas fa-globe"></i></span>
...
export default {
  ...
  methods: {
    async carregarDados() {
      ...
      await this.buscarEstadosDaApi();
      if (propriedadeSelecionada) {
        await this.buscarPropriedadeDaApi(propriedadeSelecionada);
        if (this.formData.estado) {
          await this.buscarCidadesPorEstado(this.formData.estado);
        }
      }
    },
    async buscarCidadesPorEstado(estadoId) {
      try {
        ...
        if (this.propriedade) {
          this.formData.cidade = this.propriedade.cidade;
        }
      } catch (error) {
