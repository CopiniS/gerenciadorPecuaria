        ...
        >
          <div class="col-auto d-flex align-items-center">
            ...
            <DateRangePicker
              ...
              :startDate="filtro.dataMovimentacaoInicio"
              :endDate="filtro.dataMovimentacaoFim"
              @update:startDate="(val) => (filtro.dataMovimentacaoInicio = val)"
              @update:endDate="(val) => (filtro.dataMovimentacaoFim = val)"
            />
          </div>
          <div class="col-auto d-flex align-items-center">
            <label for="produto" class="form-label me-2"
              >Piquete de Origem</label
            >
            <input
              type="text"
              class="form-control input-consistente"
              id="piqueteOrigem"
              v-model="filtro.piqueteOrigem"
            />
          </div>
          <div class="col-auto d-flex align-items-center">
            <label for="produto" class="form-label me-2"
              >Piquete de Destino</label
            >
            <input
              type="text"
              class="form-control input-consistente"
              id="piqueteDestino"
              v-model="filtro.piqueteDestino"
            />
          </div>
          <div class="col-auto d-flex align-items-center">
            ...
            <RelatorioPdf
              ...
              :dados="
                movimentacoes.map((movimentacao) => [
                  movimentacao.piqueteOrigem.propriedade.nome,
                  movimentacao.piqueteOrigem.nome,
                  movimentacao.piqueteDestino.propriedade.nome,
                  movimentacao.piqueteDestino.nome,
                  movimentacao.animal.brinco,
...
export default {
  ...
  methods: {
    ...
    async apagarMovimentacao() {
      try {
        const response = await api.delete(
          `http://127.0.0.1:8000/movimentacoes/${this.formData.id}/`,
          {}
        );
    ...
    //FILTROS----------------------------------------------------------------------------------------------------------------------------------------------------
    aplicarFiltro() {
      const propriedadeAtualNome = localStorage.getItem(
        "propriedadeSelecionada"
      );
      this.movimentacoes = this.movimentacoesDaApi.filter((movimentacao) => {
        return (
          new Date(movimentacao.dataMovimentacao) >=
            new Date(this.filtro.dataMovimentacaoInicio || "1970-01-01") &&
          new Date(movimentacao.dataMovimentacao) <=
            new Date(this.filtro.dataMovimentacaoFim || "9999-12-31") &&
          movimentacao.animal.brinco.includes(this.filtro.animal) &&
          movimentacao.piqueteOrigem.nome.includes(this.filtro.piqueteOrigem) &&
          movimentacao.piqueteDestino.nome.includes(
            this.filtro.piqueteDestino
          ) &&
          ((this.filtro.tipo == "entrada" &&
            movimentacao.piqueteOrigem.propriedade.id != propriedadeAtualNome &&
            movimentacao.piqueteDestino.propriedade.id ==
              propriedadeAtualNome) ||
            (this.filtro.tipo == "saida" &&
              movimentacao.piqueteOrigem.propriedade.id ==
                propriedadeAtualNome &&
              movimentacao.piqueteDestino.propriedade.id !=
                propriedadeAtualNome) ||
            (this.filtro.tipo == "interna" &&
              movimentacao.piqueteOrigem.propriedade.id ==
                propriedadeAtualNome &&
              movimentacao.piqueteDestino.propriedade.id ==
                propriedadeAtualNome) ||
    ...
    achaTipo(movimentacao) {
      if (
        movimentacao.piqueteOrigem.propriedade.id ==
          localStorage.getItem("propriedadeSelecionada") &&
        movimentacao.piqueteDestino.propriedade.id ==
          localStorage.getItem("propriedadeSelecionada")
      ) {
        return "Interna";
