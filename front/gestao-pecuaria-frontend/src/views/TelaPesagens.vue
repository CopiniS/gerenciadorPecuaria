        ...
        >
          <div class="col-auto d-flex align-items-center">
            ...
            <DateRangePicker
              ...
              :startDate="filtro.dataPesagemInicio"
              :endDate="filtro.dataPesagemFim"
            ...
            <tbody>
              <tr v-for="(pesagem, index) in pesagens" :key="index">
                ...
                <td>
                  ...
                  <button
                    @click="confirmarExclusaoPesagem(pesagem)"
                    class="btn-acoes btn-sm"
  ...
  methods: {
    ...
    aplicarFiltro() {
      this.pesagens = this.pesagensDaApi.filter((pesagem) => {
        return (
          new Date(pesagem.dataPesagem) >=
            new Date(this.filtro.dataPesagemInicio || "1970-01-01") &&
          new Date(pesagem.dataPesagem) <=
            new Date(this.filtro.dataPesagemFim || "9999-12-31") &&
          (pesagem.peso >= parseFloat(this.filtro.pesoInicio) ||
            this.filtro.pesoInicio == "") &&
          (pesagem.peso <= parseFloat(this.filtro.pesoFim) ||
            this.filtro.pesoFim == "") &&
