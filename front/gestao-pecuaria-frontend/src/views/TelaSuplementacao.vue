    ...
    <h2>Lista de Suplementações</h2>
      ...
      <div class="d-flex align-items-start table-container flex-column">
        ...
        >
          ...
          <div class="col-auto d-flex align-items-center">
            <label for="dataInicial" class="form-label me-2"
              >Data Inicial</label
            >
            <DateRangePicker
              ref="dateRangePicker"
              class="input-consistente"
              :startDate="filtro.dataInicialInicio"
              :endDate="filtro.dataInicialFim"
          ...
          <div class="col-auto d-flex align-items-center">
            ...
            <DateRangePicker
              class="input-consistente"
              :startDate="filtro.dataFinalInicio"
              :endDate="filtro.dataFinalFim"
      ...
      <div>
        <div class="table-container">
          <div class="button-container">
            ...
            <RelatorioPdf
              titulo="Relatório de Suplementação"
              :cabecalho="[
              ...
              :dados="
                suplementacoes.map((suplementacao) => [
                  suplementacao.piquete.nome,
                  suplementacao.produto.nome,
                  suplementacao.quantidade,
                  formatarData(suplementacao.dataInicial),
                  formatarData(suplementacao.dataFinal) || '-',
                ])
              "
            />
          </div>
          <table class="table table-bordered">
            ...
            <tbody>
              <tr v-for="(suplementacao, index) in suplementacoes" :key="index">
                ...
                <td
                  :class="{
                    'status-andamento': !suplementacao.dataFinal,
                    'status-finalizada': suplementacao.dataFinal,
                  }"
                >
                  {{ suplementacao.dataFinal ? "Finalizada" : "Em Andamento" }}
                </td>
                <td>
                  <button
                    v-if="!suplementacao.dataFinal"
                    @click="acessarFinalizacao(suplementacao)"
                    class="btn-acoes btn-sm"
                    title="Finalizar Suplementação"
                  >
                    <i class="fas fa-check"></i>
                  </button>
                  <button
                    @click="acessarEdicao(suplementacao)"
                    class="btn-acoes btn-sm"
                    title="Editar Suplementação"
                  >
                    <i class="fas fa-edit"></i>
                  </button>
                  <button
                    @click="confirmarExclusao(suplementacao)"
                    class="btn-acoes btn-sm"
  ...
  methods: {
    ...
    async buscarSuplementacoesDaApi() {
      try {
        const response = await api.get(
          "http://127.0.0.1:8000/suplementacoes/",
          {}
        );
    ...
    aplicarFiltro() {
      this.suplementacoes = this.suplementacoesDaApi.filter((suplementacao) => {
        return (
          new Date(suplementacao.dataInicial) >=
            new Date(this.filtro.dataInicialInicio || "1970-01-01") &&
          new Date(suplementacao.dataInicial) <=
            new Date(this.filtro.dataInicialFim || "9999-12-31") &&
